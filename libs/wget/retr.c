...
GetFileCallBackFunc g_callBackGetFile;
...
{
  int ret = 0;
  double percent=0,percent_tmp=0;
  ...
  while (!exact || (sum_read < toread))
	...
	limit_bandwidth (ret, timer);
      ...
      if(g_callBackGetFile != NULL){
         if (progress){
	   progress_update (progress, ret, ptimer_read (timer));

	   percent_tmp=floor(100.0 * (startpos + sum_read) / (startpos + toread));
	   ...
	   if(percent < percent_tmp){ 
		 ...
		 *   Added by Yusop */
                g_callBackGetFile((int)percent,DS_DOWN);
	   }
       }
    }	
#ifdef WINDOWS
...
void SetGetFileCallBack(GetFileCallBackFunc callBackFunc)
{
	g_callBackGetFile = callBackFunc;
}
